...
4. The GitHub Actions scheduler invokes `pnpm briefing` daily at 5:00 AM UTC and can also be run manually from the Actions tab. The workflow uses `pi --provider openai-codex --model gpt-5.4-mini`, so the repository needs an `OPENAI_API_KEY` secret.
