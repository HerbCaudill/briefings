...
**Pipeline files:** `scripts/news-briefing/` — source config, extraction helpers, candidate briefing builder, missing-briefing detection, synthesis helpers, and the repo-owned scheduler entrypoint. `.github/workflows/daily-briefing.yml` runs `pnpm briefing` daily at 5:00 AM UTC, supports manual dispatch, and uses `pi --provider openai --model gpt-5.1-codex` with the `OPENAI_API_KEY` repository secret.
